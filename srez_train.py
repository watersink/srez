...
def _summarize_progress(train_data, feature, label, gene_output, batch, suffix, max_samples=8):
    ...
    size = [label.shape[1], label.shape[2]]#[64,64]

    nearest = tf.image.resize_nearest_neighbor(feature, size)
    nearest = tf.maximum(tf.minimum(nearest, 1.0), 0.0)#(16,64,64,3)

    bicubic = tf.image.resize_bicubic(feature, size)
    bicubic = tf.maximum(tf.minimum(bicubic, 1.0), 0.0)#(16,64,64,3)

    clipped = tf.maximum(tf.minimum(gene_output, 1.0), 0.0)#(16,64,64,3)
    ...
    filename = 'batch%06d_%s.jpg' % (batch, suffix)
